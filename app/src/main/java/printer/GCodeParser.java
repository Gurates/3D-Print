...
public class GCodeParser {
    ...
    public void parseLine(String line) {
        ...
        if (line.contains("Done printing file") && state.isPrinting) {
            ...
            String msg = "✅ <b>BASKI TAMAMLANDI!</b>\n\n" +
                         ...
                         "🧵 <b>Harcanan Filament:</b> " + String.format("%.2f m", filamentUsedM) + "\n" +
                         "⏱ <b>Toplam Süre:</b> " + state.elapsedTime;
        ...
        if (errorMatcher.find() && state.isPrinting) {
            ...
            String msg = "❌ <b>BASKI DURDURULDU (HATA)!</b>\n\n" +
                         "🖨 <b>Yazıcı:</b> " + state.displayName + "\n" +
                         "📦 <b>Model:</b> " + state.currentPrintName + "\n" +
                         "⚠️ <b>Hata Nedeni:</b> <code>" + reason + "</code>\n" +
                         "⏱ <b>Geçen Süre:</b> " + state.elapsedTime + "\n\n" +
                         "<i>Lütfen üretim alanını kontrol edin!</i>";
            TelegramNotifier.sendMessage(msg);
        ...
        if (filamentMatcher.find()) {
            this.filamentUsedM = Double.parseDouble(filamentMatcher.group(1));
            return;
        }
        ...
        if (sdMatcher.find()) {
            ...
            if (total > 0) {
                state.progressPercent = (int) ((cur / total) * 100);
            }
            return;
        }
    ...
    private int parseElapsedToSeconds(String elapsed) {
        if (elapsed == null || elapsed.equals("00:00:00") || elapsed.isEmpty()) return 0;
        try {
            if (elapsed.contains(":")) {
